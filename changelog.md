...
- Menambahkan portal agregator berita nasional dan pemerintahan (`/berita`) bergaya daily.dev dengan sindikasi 14 kanal RSS media terpercaya (ANTARA News, CNN Indonesia, CNBC Indonesia, Tempo.co, Detikcom, Republika). Fitur mencakup pencarian judul/topik instan, filter kategori (Pemerintahan, Hukum & Korupsi, Ekonomi & Kebijakan, Nasional), filter sumber media, bookmarking lokal (baca nanti) di peramban, pergantian tampilan kisi (grid) dan daftar (compact list), tombol sinkronisasi segar feed, serta penautan langsung ke sumber asli berita.
